...
{
    return fluid_defpreset_noteon((fluid_defpreset_t *)preset->data, synth, chan,
                                  key, vel);
}
...
static int fluid_cached_sampledata_load(char *filename, unsigned int samplepos,
                                        unsigned int samplesize,
                                        short **sampledata, int try_mlock)
{
    fluid_file fd = NULL;
    ...
    for (cached_sampledata = all_cached_sampledata; cached_sampledata;
        ...
        if (cached_sampledata->samplesize != samplesize) {
            FLUID_LOG(FLUID_ERR, "Cached size of soundfont doesn't match actual size "
                      "of soundfont (cached: %u. actual: %u)",
    ...
    cached_sampledata = (fluid_cached_sampledata_t *)FLUID_MALLOC(
                            sizeof(fluid_cached_sampledata_t));
        ...
        if (sampledata == cached_sampledata->sampledata) {
            ...
            if (cached_sampledata->num_references == 0) {
                if (cached_sampledata->mlock) {
                    fluid_munlock(cached_sampledata->sampledata,
                                  cached_sampledata->samplesize);
                }
...
fluid_defsfont_t *new_fluid_defsfont(fluid_settings_t *settings)
{
    ...
    /* Initialise preset cache, so we don't have to call malloc on program
       changes.
    ...
    sfont->preset_stack =
        FLUID_ARRAY(fluid_preset_t *, sfont->preset_stack_capacity);
...
int fluid_defsfont_load_sampledata(fluid_defsfont_t *sfont)
{
    return fluid_cached_sampledata_load(sfont->filename, sfont->samplepos,
                                        sfont->samplesize, &sfont->sampledata,
                                        sfont->mlock);
}
        ...
        if (fluid_preset_zone_inside_range(preset_zone, key, vel)) {
            ...
            while (inst_zone != NULL) {
                ...
                if (fluid_inst_zone_inside_range(inst_zone, key, vel) &&
                    ...
                    /* Store the ID of the first voice that was created by this noteon
                     * event.
                     * Exclusive class may only terminate older voices.
                     * That avoids killing voices, which have just been created.
                     * (a noteon event can create several voice processes with the same
                     * exclusive
                                 ...
                                 fluid_defsfont_t *sfont)
{
    ...
    if (FLUID_STRLEN(sfpreset->name) > 0) {
        ...
        FLUID_SPRINTF(preset->name, "Bank%dPreset%d", sfpreset->bank,
                      sfpreset->prenum);
    }
...
fluid_preset_zone_t *new_fluid_preset_zone(char *name)
{
     ...
     * found
     * in the sound font).
     * This also sets the generator values to default, but that is of no concern
     * here.*/
                                   ...
                                   fluid_defsfont_t *sfont)
{
    ...
    for (count = 0, r = sfzone->gen; r != NULL; count++) {
        ...
        case GEN_VELRANGE:
            ...
            break;
        default:
            /* FIXME: some generators have an unsigne word amount value but i don't
             * know which ones */
    ...
    if ((sfzone->instsamp != NULL) && (sfzone->instsamp->data != NULL)) {
        ...
        if (fluid_inst_import_sfont(zone->inst, (SFInst *)sfzone->instsamp->data,
                                    sfont) != FLUID_OK) {
            return FLUID_FAILED;
        }
    }
    ...
    for (count = 0, r = sfzone->mod; r != NULL; count++) {
        ...
        mod_dest->next =
            NULL; /* pointer to next modulator, this is the end of the list now.*/
        ...
        mod_dest->src2 =
            mod_src->amtsrc &
            127; /* index of source 2, seven-bit value, SF2.01 section 8.2, p.50 */
        mod_dest->flags2 = 0;
                                 ...
                                 fluid_defsfont_t *sfont)
{
    ...
    for (count = 0, r = sfzone->mod; r != NULL; count++) {
        ...
        /* *** Amount source *** */
        mod_dest->src2 = mod_src->amtsrc & 127; /* index of source 2, seven-bit
                                               value, SF2.01 section 8.2, page
                                               50 */
        mod_dest->flags2 = 0;
         ...
         * The order of modulators will make a difference, at least in an instrument
         * context:
         * The second modulator overwrites the first one, if they only differ in
         * amount. */
        if (count == 0) {
            zone->mod = mod_dest;
        } else {
                              ...
                              fluid_defsfont_t *sfont)
{
    ...
    if (sample->end - sample->start < 8) {
        ...
        /*        FLUID_LOG(FLUID_WARN, "Fixing sample %s: at least 8 data points
         * required before loop start", sample->name);     */
        /*        sample->loopstart = sample->start + 8; */
        /*      } */
        /*      if (sample->loopend > sample->end - 8) { */
        /*        FLUID_LOG(FLUID_WARN, "Fixing sample %s: at least 8 data points
         * required after loop end", sample->name);     */
...
SFData *sfload_file(const char *fname)
{
    ...
    /* get size of file */
    if (!err && fseek(fd, 0L, SEEK_END) == -1) { /* seek to end of file */
        err = TRUE;
        FLUID_LOG(FLUID_ERR, _("Seek to end of file failed"));
    }
    if (!err && (fsize = ftell(fd)) == -1) { /* position = size */
        err = TRUE;
...
static int process_info(int size, SFData *sf, FILE *fd)
{
    ...
    while (size > 0) {
        ...
        if (id == IFIL_ID) { /* sound font version chunk? */
            if (chunk.size != 4)
                return (
                           gerr(ErrCorr, _("Sound font version info chunk has invalid size")));
            ...
            if ((id != ICMT_ID && chunk.size > 256) || (chunk.size > 65536) ||
                (chunk.size % 2))
                return (gerr(ErrCorr, _("INFO sub chunk %.4s has invalid chunk size"
                                        " of %d bytes"),
                             &chunk.id, chunk.size));
    ...
    while (p) { /* traverse through presets */
        ...
        while (p2) { /* traverse preset's zones */
            ...
            if (pz) { /* if not first zone */
                if (genndx < pgenndx)
                    return (
                               gerr(ErrCorr, _("Preset bag generator indices not monotonic")));
                if (modndx < pmodndx)
                    return (
                               gerr(ErrCorr, _("Preset bag modulator indices not monotonic")));
                i = genndx - pgenndx;
                while (i--)
            ...
            while (p3) { /* load zone's generators */
                ...
                if (genid == Gen_KeyRange) { /* nothing precedes */
                    ...
                    ((SFZone *)(p2->data))->instsamp =
                        FLUID_INT_TO_POINTER(genval.uword + 1);
    ...
    while (p) { /* traverse through inst */
        ...
        while (p2) { /* load this inst's zones */
            ...
            if (pz) { /* if not first zone */
                if (genndx < pgenndx)
                    return (
                               gerr(ErrCorr, _("Instrument generator indices not monotonic")));
                if (modndx < pmodndx)
                    return (
                               gerr(ErrCorr, _("Instrument modulator indices not monotonic")));
                i = genndx - pgenndx;
                while (i--)
                    pz->gen = fluid_list_prepend(pz->gen, NULL);
                i = modndx - pmodndx;
